    ...
    function initCursor(){
      ...
      if(!dot||matchMedia("(max-width:760px)").matches) return;
      ...
      function spawnEmber(x,y,dx,dy){
        ...
        ember.style.left=`${x-dx*0.2+(Math.random()-0.5)*12}px`;
        ember.style.top=`${y-dy*0.2+(Math.random()-0.5)*12}px`;
        document.body.appendChild(ember);
        setTimeout(()=>ember.remove(),560);
      }
      ...
      addEventListener("mousemove",e=>{
        ...
        spawnEmber(pointerX,pointerY,dx,dy);
      });

      addEventListener("mouseout",()=>document.body.classList.add("cursor-hidden"));
      animateCursor();
    }
